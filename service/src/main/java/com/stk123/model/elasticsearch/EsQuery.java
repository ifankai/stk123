...
import com.stk123.model.elasticsearch.Condition;
import com.stk123.model.elasticsearch.QueryParamData;
import com.stk123.model.elasticsearch.RangeData;
import com.stk123.model.elasticsearch.Condition.*;
import com.stk123.model.elasticsearch.Type.FieldType;
import com.stk123.model.elasticsearch.Type.BoolType;
import com.stk123.model.elasticsearch.Type.SortType;
import com.stk123.model.elasticsearch.Type.GeoRelationType;
import com.stk123.model.elasticsearch.Type.AggType;
...
public class EsQuery {
    ...
    // 搜索建议
    private static int SUGGEST_DEFAULT_SIZE = 5;
     ...
     * @return
     */
    public static RangeQueryBuilder getRangeQuery(RangeCondition rangeQueryCondition){
     ...
     * @param boolType
     */
    public static void setBoolQuery(BoolQueryBuilder boolQueryBuilder, QueryBuilder queryBuilder, BoolType boolType){
        switch (boolType){
            // 且
            case MUST:
     ...
     * @return
     */
    public static SearchRequest createSearchRequest(QueryParamData queryParamData) throws IOException {
        ...
        for(Condition condition : conditions){
            ...
            switch (fieldType){
                ...
                case TERM:
                    ...
                    setBoolQuery(filterBoolQueryBuilder, termsQueryBuilder, termCondition.getBoolType());
                    break;
                // 范围查询
                case RANGE:
                    RangeCondition rangeCondition = (RangeCondition)condition;
                    RangeQueryBuilder rangeQueryBuilder = getRangeQuery(rangeCondition);
                    setBoolQuery(filterBoolQueryBuilder, rangeQueryBuilder, rangeCondition.getBoolType());
                    break;
                // 搜索建议
                case SUGGEST:
                ...
                case GEO:
                    ...
                    setBoolQuery(filterBoolQueryBuilder, geoShapeQueryBuilder, geoCondition.getBoolType());
                    break;
            }
        }
        if(aggList.size() > 0 ){
        ...
        searchSourceBuilder.postFilter(filterBoolQueryBuilder);
     ...
     * @return
     */
    private static AggregationBuilder getAggrationBuilder(List<AggregationBuilder> aggList){
        AggregationBuilder aggregationBuilder = null;
     ...
     * @return
     */
    private static Condition[] vaildConditions(Condition[] conditions){
