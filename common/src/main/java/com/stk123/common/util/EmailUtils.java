	...
	*/}
	...
	private static void writeFile(String title, String msg) {
		try {
			...
			String path = CommonConstant.PATH_DAILYREPORT_EMAIL_BACKUP +CommonUtils.getToday()+"\\";
			String indexFile = path + "index.html";
			String linkFile = path + "link.html";
			File file = new File(path);
			if(!file.exists()){
				file.mkdir();
				File index = new File(CommonConstant.PATH_DAILYREPORT_EMAIL_BACKUP + "index.html");
				FileUtils.copyFileToDirectory(index, file);
				File link = new File(CommonConstant.PATH_DAILYREPORT_EMAIL_BACKUP + "link.html");
