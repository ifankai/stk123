...
/**
 * OVERVIEW: Edges in images are usually to defined to be 1 pixel thick. Thus,
 * we cannot simply look for pixels at which place the change in intensity is
 * sufficiently large (greater than a threshold). We must also suppress
 * non-maximum pixels. Essentially, we calculate the edge direction (direction
 * in which gradient of pixel intensity is greatest). We round the edge
 * direction to the nearest 45 degrees. (i, j) is suppressed if either of the
 * pixels one unit in the edge direction have greater gradient magnitudes than
 * (i, j).
 * 
...
public class NonMaximumSuppression {
	...
	public enum EdgeDirection {
		...
		VERTICAL, HORIZONTAL, DIAG_LEFT_UP, DIAG_RIGHT_UP;
		...
		/**
		 * Gets direction for non-maximum suppression from G_x and G_y. Handles case
		 * when G_x == 0 and can't calculate arctan.
		 * 
		 * @param G_x
		 * @param G_y
		 * @return
		...
		public static EdgeDirection getDirection(double G_x, double G_y) {
			return (G_x != 0) ? getDirection(Math.atan(G_y / G_x))
					: ((G_y == 0) ? EdgeDirection.HORIZONTAL : EdgeDirection.VERTICAL);
		}
	 ...
	 * @return
	 */
	public static boolean nonMaximumSuppression(double[][] mag, EdgeDirection angle, int i, int j) {
	...
	/**
	 * Get coordinates of the two points needed to check for non-maximum
	 * suppression.
	 * 
	 * @param d
	 * @param i
