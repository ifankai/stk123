...
/*
 * Source:
 * http://www.programcreek.com/java-api-examples/index.php?source_dir=trackanalyzer-master/at/ofai/music/util/Peaks.java
 */
...
/**
 * 
 * @author Simon Dixon, updated by Azim Ahmadzadeh and Dustin Kempton, Data
...
public abstract class Peaks {
	 ...
	 * @return list of peak indexes
	 */
	public static LinkedList<Integer> findPeaks(double[] data, int width, double threshold, double decayRate,
			boolean isRelative) {
	...
	public static boolean overThreshold(double[] data, int index, int width, double threshold, boolean isRelative,
			double av) {
		if (debug)
		...
		if (isRelative) {
			...
			while (iStart < iStop)
				sum += data[iStart++];
			if (debug)
				System.out.printf("    %6.3f    %6.3f   ", sum / count, data[index] - sum / count - threshold);
