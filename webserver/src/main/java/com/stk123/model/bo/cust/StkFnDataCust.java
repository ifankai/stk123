package com.stk123.model.bo.cust;
...
public class StkFnDataCust extends StkFnData implements TableCell {
	...
	public Double getRateOfYear(boolean isFnYear) throws Exception{
		...
		String MMdd = "";
		try {
			year = ServiceUtils.formatDate(ServiceUtils.sf_ymd2.parse(this.getFnDate()), ServiceUtils.sf_yyyy);
			MMdd = ServiceUtils.formatDate(ServiceUtils.sf_ymd2.parse(this.getFnDate()), ServiceUtils.sf_MMdd);
	...
	public String getFnValueToString(){
		Double dValue = this.getFnValue();
		if(dValue == null)return CommonConstant.MARK_DOUBLE_HYPHEN;
		if(this.getStkFnType() != null){
			String fnValue = ServiceUtils.number2String(dValue, this.getStkFnType().getPrecision());
			if(this.getStkFnType().getIsPercent() == 1){
				return fnValue + CommonConstant.MARK_PERCENTAGE;
			}
			return fnValue;
		}
		return ServiceUtils.number2String(dValue, 2);
	}
	
	public StkFnDataCust getBefore() throws Exception {
		if(this.before == null){
			this.before = this.getFnData(this.getStkFnType().getType().toString(), ServiceUtils.getPrevQuarter(this.getFnDate()));
		}
		return before;
	}
