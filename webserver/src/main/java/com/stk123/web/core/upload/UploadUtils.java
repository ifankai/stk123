...
public class UploadUtils {
	...
	public static UploadResult upload(String filePath, String fileName, HttpServletRequest request) throws ServletException, IOException {
		...
		int index = contentType.lastIndexOf(CommonConstant.MARK_EQUALS);
		...
		if (((double)formDataLength/1024/1024) >= MAX_MB) {
			return new UploadResult(false, null, "文件大小：" + ServiceUtils.numberFormat(((double)formDataLength/1024/1024), 2)	+ "MB - 超过最大限制：" + MAX_MB + "MB");
		}
		...
		String m_requestDataString = new String (data, CommonConstant.ENCODING_ISO_8859_1);
		...
		index = m_fileName.indexOf(CommonConstant.MARK_DOUBLE_QUOTATION);
		if (index < 1){
		...
		if(m_fileName.indexOf(CommonConstant.MARK_DOT) > 0){
			fileType = m_fileName.substring(m_fileName.indexOf(CommonConstant.MARK_DOT)+1, m_fileName.length());
			if(!CommonConstant.UPLOAD_IMAGE_TYPES.contains(fileType.toLowerCase())){
				fileType = null;
			}
		}
		if(fileType == null){
			return new UploadResult(false, null, "请上传指定的图片类型："+CommonConstant.UPLOAD_IMAGE_TYPES);
		}

		int posStart = m_requestDataString.indexOf(STR_FILENAME);
		posStart = m_requestDataString.indexOf(CommonConstant.MARK_NEW_LINE,posStart)+1;	//	end of Context-Disposition
		posStart = m_requestDataString.indexOf(CommonConstant.MARK_NEW_LINE,posStart)+1;	//	end of Content-Type
		posStart = m_requestDataString.indexOf(CommonConstant.MARK_NEW_LINE,posStart)+1;	//	end of empty line
		...
		if(fileName.indexOf(CommonConstant.MARK_DOT) < 0){
			fileName += CommonConstant.MARK_DOT + fileType;
		}
