...
import com.stk123.model.bo.StkCare;
...
import com.stk123.common.util.JdbcUtils.IngoreCaseHashMap;
...
public class StkAction {
	...
	public String perform() throws Exception {
		...
		String code = request.getParameter(CommonConstant.PARAMETER_S);
	...
	public void getCare() throws Exception {
		...
		for(StkCare care : cares){
			...
			map.add(ServiceUtils.wrapCodeAndNameAsHtml(index.getCode(), index.getName()));
			map.add(ServiceUtils.number2String(index.getTotalMarketValue(),2));
			map.add(care.getType());
			map.add("<a target='_blank' href='"+care.getUrl()+"'>"+care.getInfo()+"</a>");
			map.add(ServiceUtils.formatDate(care.getInfoCreateTime(), ServiceUtils.sf_ymd) );
			...
			map.add(ServiceUtils.formatDate(care.getInsertTime(), ServiceUtils.sf_ymd));
			list.add(map);
		}
	...
	public void getMonitor() throws Exception {
		...
		for(StkMonitor care : ms){
			...
			map.add(ServiceUtils.formatDate(care.getParam3(),ServiceUtils.sf_ymd2, ServiceUtils.sf_ymd) );
			map.add(care.getParam2());
			//map.put("close", index.getK().getClose());
			map.add(ServiceUtils.formatDate(care.getParam4(),ServiceUtils.sf_ymd2, ServiceUtils.sf_ymd));
	...
	public void care() throws Exception {
		...
		params.add(url);
		params.add(memo);
		params.add(new Timestamp(ServiceUtils.sf_ymd.parse(createtime).getTime()));
		params.add(param1);
		params.add(param2);
	...
	public void stkValueHistory() throws Exception{
		...
		String json = "{\"data\":"+JsonUtils.getJsonString4JavaPOJO(list)+CommonConstant.MARK_BRACE_RIGHT;
		sc.setResponse(json);
	}
	...
	public void listRelatedStk() throws Exception{
		...
		json.append("{\"stk1\":").append(json1).append(CommonConstant.MARK_COMMA).append("\"stk2\":").append(json2).append(CommonConstant.MARK_BRACE_RIGHT);
	...
	public List<Map> getRelatedStk(Connection conn, String code, boolean includeCompanyInfo) throws Exception{
		...
		String sZhuyin = CommonConstant.MARK_EMPTY;
		...
		if(includeCompanyInfo){
			keyword.append(StringUtils.join(kws, CommonConstant.MARK_COMMA)).append(CommonConstant.MARK_COMMA).append(sZhuyin).append(CommonConstant.MARK_COMMA).append(index.getStock().getCompanyProfile());
		}else{
		...
		for(Document stk : stks){
			...
			Map map = new HashMap();
			map.put(CommonConstant.JSON_CODE, index.getCode());
			map.put(CommonConstant.JSON_NAME, index.getName());
			jsonList.add(map);
		}
		return jsonList;
	}
	...
	public void search() throws Exception{
		...
		HttpServletRequest request = sc.getRequest();
		Connection conn = StkContext.getConnection();
		String k = request.getParameter(CommonConstant.PARAMETER_K);
		...
		for(final Map<String,String> map : list){
			...
			Runnable run = new Runnable() {
				public void run() {
					try{
						try {
							int n = BaiduSearch.getBaiduNewsCount(ServiceUtils.addDay(new Date(), -7),map.get("keyword"),new Boolean(map.get("intitle")).booleanValue());
	...
	public void listNotice() throws Exception {
		...
		String json = JsonUtils.getJsonString4JavaPOJO(subNotices, CommonConstant.DATE_FORMAT_YYYY_MM_DD);
	...
	public void getEarningsForecast() throws Exception {
		...
		for(Map m : ms){
			...
			l.add(ServiceUtils.wrapCodeAndNameAsHtml(index.getCode(), index.getName()));
	...
	public void getXueqiuArticle() throws Exception{
		...
		for(StkText article : texts){
			...
			l.add(text);
			l.add(reply);
			l.add(ServiceUtils.sf_ymd9.format(article.getInsertTime()));
			list.add(l);
		}
		String json = JsonUtils.getJsonString4JavaPOJO(list);
		sc.setResponse("{\"data\":" + json + "}");
