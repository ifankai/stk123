	...
	public void run(Connection conn, String today) throws Exception {
		...
		if(results1.size()+results2.size() > 0){
			...
			for(Index result : results1){
				...
				data.add(result);
				data.add(ServiceUtils.numberFormat(result.changePercent, 2));
				datas1.add(data);
			}
			...
			if(results1.size()+results2.size() > 0){
				...
				EmailUtils.sendAndReport("模型5-突破趋势线,个数："+(results1.size()+results2.size())+",日期:"+today,
						"突破长期趋势线:<br>"+TaskUtils.createHtmlTable(today, datas2, addtitle)
						+ "<br><br>" +
						"突破短期趋势线:<br>"+TaskUtils.createHtmlTable(today, datas1, addtitle)
						);
			}
		}
