...
import com.stk123.task.schedule.NoticeRobot;
...
public class AStkTools {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			List<StkFnType> fnTypes = JdbcUtils.list(conn, "select * from stk_fn_type where market=1", StkFnType.class);
			...
			for(Stk stk : stks){
				try{
					...
					InitialData.initFnDataTTM(conn, new Date(), index, fnTypes);
	...
	public static void test(Index index) throws Exception {
		String page = HttpUtils.get("http://www.cninfo.com.cn//disclosure/fulltext/stocks/fulltext1y/cninfo/"+index.getCode()+".js?ver="+ServiceUtils.formatDate(new Date(), ServiceUtils.sf_ymd12), "gb2312");
		if("404".equals(page)){
			return ;
		}
		...
		for(List item : list){
			...
			if(title.contains("年度报告") || title.contains("季度报告全文")){
				...
				if("pdf".equalsIgnoreCase(fileType)){
					...
					int total = 0;
					for(String kw : kws){
						int n = ServiceUtils.getMatchAllStrings(spdf, kw).size();
