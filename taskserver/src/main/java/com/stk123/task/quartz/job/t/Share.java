package com.stk123.task.quartz.job.t;
...
import com.stk123.task.quartz.job.t.K.Calc;
...
public class Share {
	...
	public boolean addK(K k){
		...
		if(k.getBefore() != null){
			...
			k.setChange(ServiceUtils.numberFormat(k.getClose() - k.getBefore().getClose(), 2));
	...
	public void initK(String today) throws Exception {
		...
		for(int i=0; i<data.size(); i++){
			...
			if(!StringUtils.replace(today, "-", "").equals(date)){
				...
				for(String d : dt){
					...
					k.setVolume(v);
					k.setAmount(a);
					k.setTime(ServiceUtils.sf_ymd12.parse(date+s[0]+"00"));
					this.addK(k);
				}
			}
		}
		...
		do{
			...
			for(int i=0; i<ss.length; i++){
				...
				k.setTime(ServiceUtils.sf_ymd9.parse(today +" "+ s[1]));
