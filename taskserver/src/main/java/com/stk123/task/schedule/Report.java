...
public class Report {
		...
		for(Stk stk : stks){
			try{
				...
				if(maMin * 1.1 >= maMax /*&& (index.isGrowth() || index.isPotential())*/){
					...
					if(maxHsl >= 400 
							&& ServiceUtils.getDaysBetween(ServiceUtils.sf_ymd2.parse(today), ServiceUtils.sf_ymd2.parse(maxK.getDate())) > 60){
		...
		for(Stk stk : stks){
			try{
				...
				if(index.getKs().size() >= 250){
					K startK = index.getK(ServiceUtils.getToday(), 250);
					K highK = index.getKByHHV(startK.getDate(), ServiceUtils.getToday());
					K todayK = index.getK();
					...
					if(highK.getDate().compareTo(last20dayK.getDate()) <= 0
						...
						K lowK = index.getKByLLV(startK.getDate(), ServiceUtils.getToday());
	...
	public static void testRS(Connection conn) throws Exception {
		...
		int cnt = 50;
		IndexUtils.sortByCloseChange(context.indexs, ServiceUtils.getToday(), 20);
	...
	public static void financeForecast(Connection conn, String url) throws Exception {
		...
		for(List data:datas){
			...
			String fnDate = (ServiceUtils.YEAR-1)+"1231";
			...
			if(fnData != null && fnData.get(index.FN_JLR) != null){
				...
				data.add(ServiceUtils.number2String(d,2));
			}else{
				data.add("0");
			}
	...
	public static void financeActual(Connection conn, String date) throws Exception {
		...
		for(Stk stk : stks){
			...
			String year = ServiceUtils.formatDate(ServiceUtils.sf_ymd2.parse(date), ServiceUtils.sf_yyyy);
			...
			if(fnData.get(index.FN_JLR).getFnValue().doubleValue() > 0 &&
				...
				data.add(ServiceUtils.formatDate(fnData.get(index.FN_JLR).getInsertTime()));
				...
				data.add(ServiceUtils.number2String(fnData.get(index.FN_JLR).getFnValue() / lastFnData.get(index.FN_JLR).getFnValue() * 100,2));
				data.add(StringUtils.join(index.getIndustryName(), "/"));
				datas.add(data);
			}
		}
